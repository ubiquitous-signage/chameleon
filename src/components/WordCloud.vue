<template>
  <div class="wordcloud">
    ...
    :style="{fontSize: word.count + 'em', color: 'rgba(10, 30, 60, ' +  (word.count / maxCount) + ')', zIndex: word.count, left: (centeredRandom() * 100) + '%', top: (centeredRandom() * 100) + '%', transform: 'translate(-50%, 0) rotate(' + (Math.random() * 120 - 60) + 'deg)'}">
      {{ word.text }}
    </div>
  </div>
</template>
